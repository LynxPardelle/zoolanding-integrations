...
class Store:
    ...
    def claim(self, scope, connection_id, key, request_hash, command_id, expires_at):
    ...
    def complete(
        ...
        if code_claim is not None:
            self.code_claims[(scope.partition_key, connection_id, code_claim)] = mappings[0]["resourceId"]
    ...
    def mark_unknown(self, scope, connection_id, key, request_hash):
        self.receipts[(scope.partition_key, connection_id, key)]["status"] = "unknown"
...
class Provider:
    ...
    def create_price(self, resolved_binding, product_id, snapshot, resource_id, idempotency_key):
    ...
    def update_product_presentation(self, resolved_binding, product_id, snapshot, idempotency_key):
    ...
    def deactivate_discount(self, resolved_binding, coupon_id, promotion_code_id, target, idempotency_key):
        self.calls.append(("deactivate-discount", coupon_id, promotion_code_id, target, idempotency_key))

    def create_checkout(self, resolved_binding, lines, promotion_code_id, command_input, routes, idempotency_key):
        self.calls.append(("checkout", lines, promotion_code_id, routes, idempotency_key))
...
class StripeCommandTests(unittest.TestCase):
    ...
    def test_offer_provisions_product_and_immutable_price_then_replays_without_provider(self):
        result = self.provision_offer()
        replay = self.provision_offer()

        self.assertEqual(result, {"commandId": "command-1", "status": "accepted"})
        self.assertEqual(replay, result)
        self.assertEqual([call[0] for call in self.provider.calls], ["product", "price"])
    ...
    def test_discount_maps_coupon_and_promotion_code_and_casefolds_active_uniqueness(self):
        self.provision_offer()
        ...
        same_code["idempotencyKey"] = integration_key(
            same_code["scope"], "stripe-primary", "discount", "discount-v2", 2, content_hash
        )
        with self.assertRaisesRegex(Exception, "conflict"):
    ...
    def test_retired_discount_releases_only_its_hashed_active_code_claim(self):
        ...
        result = self.service.execute(
            "discount", command("discount", replacement)
        )
        self.assertEqual(result["status"], "accepted")

    def test_checkout_uses_only_scoped_mapped_prices_discount_and_published_routes(self):
        self.provision_offer()
        self.provision_discount()
        result = self.service.execute(
        ...
        checkout_call = next(call for call in self.provider.calls if call[0] == "checkout")
        self.assertEqual(checkout_call[1], [{"price": "price_synthetic01", "quantity": 2}])
    ...
    def test_unknown_checkout_retries_the_same_derived_key_and_safe_replay_refetches_url(self):
        self.provision_offer()
        ...
        self.assertTrue(any(call[0] == "retrieve-checkout" for call in self.provider.calls))
    ...
    def test_durable_mappings_prevent_duplicate_provider_writes_after_receipt_ttl(self):
        ...
        self.assertEqual(
            [call[0] for call in self.provider.calls].count("product"), 1
        )
        self.assertEqual(
            [call[0] for call in self.provider.calls].count("checkout"), 1
        )
        self.assertEqual(
